...
export default function Index({ auth, messages, devices, orderStatuses, userFormTemplates, supportedPlaceholders }) {
    ...
    useEffect(() => {
        if (data.action === 'FORM_SUBMISSION' && data.form_template_id) {
            ...
            axios.get(route('schedulers.potential-fields', data.form_template_id))
                .then(response => {
    ...
    useEffect(() => {
        if (data.action === 'ORDER' && data.order_criteria_type !== 'ALL' && data.status) {
            
            // Execute search only if user has typed something
            if (debouncedOrderSearch !== null && debouncedOrderSearch.length > 0) {
                axios.get(route('schedulers.search-orders', { status: data.status, search: debouncedOrderSearch }))
                    .then(response => {
                 ...
                 // Clear the list if the search box is empty, but PRESERVE target_id
                 setOrderSearchList([]);
            }
        } else {
            // Only reset the search UI fields if criteria change away from specific order
            setOrderSearchList([]);
    ...
    useEffect(() => {
        if (data.action === 'FORM_SUBMISSION' && data.submission_criteria_type === 'SPECIFIC_SUBMISSION' && data.form_template_id) {
            
            // Execute search only if user has typed something
            if (debouncedSubmissionSearch !== null && debouncedSubmissionSearch.length > 0) {
                axios.get(route('schedulers.search-submissions', { form_template_id: data.form_template_id, search: debouncedSubmissionSearch }))
                    .then(response => {
            ...
            // Only reset the search UI fields if criteria change away from specific submission
            setSubmissionSearchList([]);
    ...
    const submit = (e) => {
        ...
        post(route('schedulers.store'), payload, {
            onSuccess: () => {
                reset();
                setActiveTab('list');
            }
    ...
    const handleDelete = (message) => {
        if (confirm('Are you sure you want to delete this scheduled message?')) {
            router.delete(route('schedulers.destroy', message.id));
        }
    };
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-6">
                                                ...
                                                <label className="flex items-center space-x-3">
                                                    <input 
                                                        ...
                                                        onChange={() => {setData('order_criteria_type', 'STATUS'); setData('target_id', ''); setOrderSearchTerm('');}} 
                                                        className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" 
                                                    />
                                                ...
                                                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <select
                                                            id="status"
                                                            value={data.status}
                                                            onChange={(e) => {setData({ ...data, status: e.target.value, target_id: '' }); setOrderSearchTerm('');}}
                                                            className="block w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                                                        >
                                                    ...
                                                    {data.order_criteria_type === 'SPECIFIC_ORDER' && (
                                                        <div className="relative">
                                                            <label htmlFor="order_search_term" className="block text-sm font-medium text-gray-700 mb-2">
                                                                Specific Order (Search by ID or Name)
                                                            </label>
                                                            
                                                            {/* HIDDEN INPUT: Ensures target_id is always submitted */}
                                                            ...
                                                            <input
                                                                ...
                                                                onChange={(e) => {
                                                                    ...
                                                                    // Clear target_id if the user starts typing again
                                                            ...
                                                            {Array.isArray(orderSearchList) && orderSearchTerm.length > 0 && orderSearchList.length > 0 && (
                                                                <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-xl mt-1 max-h-60 overflow-y-auto shadow-lg">
                                                                    {orderSearchList.map(order => (
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span className="block text-xs text-gray-500">{order.mobile}</span>
                                                                        </li>
                                                                    ))}
                                                                </ul>
                                                            )}
                                        ...
                                        <div className="space-y-4">
                                                        ...
                                                        <div className="flex items-center space-x-6 h-full">
                                                            <label className="flex items-center space-x-3">
                                                                <input 
                                                                    ...
                                                                    onChange={() => {setData('submission_criteria_type', 'ALL'); setData('target_id', ''); setSubmissionSearchTerm('');}} 
                                                                    className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" 
                                                                />
                                            ...
                                            {data.submission_criteria_type === 'SPECIFIC_SUBMISSION' && data.form_template_id && (
                                                <div className="relative mt-4">
                                                    <label htmlFor="submission_search_term" className="block text-sm font-medium text-gray-700 mb-2">
                                                    ...
                                                    {Array.isArray(submissionSearchList) && submissionSearchTerm.length > 0 && submissionSearchList.length > 0 && (
                                                        <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-xl mt-1 max-h-60 overflow-y-auto shadow-lg">
                                                            {submissionSearchList.map(submission => (
                                                                ...
                                                                >
                                                                    ...
                                                                    <span className="block text-xs text-gray-500">Submitted: {new Date(submission.created_at).toLocaleDateString()}</span>
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    )}
